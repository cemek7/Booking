...
-- 5. Webhook Events Table (Enhanced for security)
CREATE TABLE IF NOT EXISTS public.webhook_events (
  ...
  CONSTRAINT webhook_events_provider_external_unique UNIQUE (provider, external_id)
);
...
-- 6. Staff Availability Table (For conflict detection)
...
-- 7. Add reconciliation columns to existing transactions table
DO $$
BEGIN
...
-- Webhook events accessible by service role only
CREATE POLICY "service_role_webhook_events" ON public.webhook_events
  FOR ALL USING (current_setting('jwt.claims.role', true) = 'service_role');

-- 9. Functions for automated cleanup
...
-- Insert default notification for risk management setup
