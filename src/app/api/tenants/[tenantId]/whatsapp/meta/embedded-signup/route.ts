...
export const POST = createHttpHandler(
  async (ctx) => {
    ...
    try {
      ...
      await upsertStoredProviderApiKey(ctx.supabase, tenantId, 'meta', accessToken);
      const { error: secretError } = await ctx.supabase
      ...
      await ctx.supabase.from('tenant_meta_connection_events').insert({
        tenant_id: tenantId,
      ...
      await ctx.supabase.from('whatsapp_configurations').update({
        meta_connection_source: 'embedded_signup',
