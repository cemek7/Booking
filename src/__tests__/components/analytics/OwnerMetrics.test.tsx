...
describe('OwnerMetrics', () => {
  ...
  describe('Rendering - Basic Structure', () => {
    it('should render without crashing', () => {
      const { container } = render(<OwnerMetrics {...defaultProps} />);
  ...
  describe('Props Handling', () => {
    it('should accept tenantId prop', () => {
      const { container } = render(<OwnerMetrics tenantId="test-tenant" />);
      expect(container).toBeInTheDocument();
    });

    it('should render with different tenant IDs', () => {
      const { rerender, container } = render(<OwnerMetrics tenantId="tenant-1" />);
      expect(container).toBeInTheDocument();

      rerender(<OwnerMetrics tenantId="tenant-2" />);
      expect(container).toBeInTheDocument();
    });
  });
  ...
  describe('Business Scope', () => {
    ...
    it('should include financial data', () => {
      const { container } = render(<OwnerMetrics {...defaultProps} />);
      // Owner metrics should contain financial indicators like $ signs
